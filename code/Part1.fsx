...
let countLookup = counts |> dict // can do counts |> Map.ofSeq also
let probabilities = features |> Array.map (fun feat -> 
                                            if countLookup.ContainsKey feat
                                            then ( (float countLookup.[feat]) / total)
                                            else 0.)
...
let getFeatureVector text = 
  ...
  let countLookup = counts |> dict
  features |> Array.map (fun feat -> 
                            if countLookup.ContainsKey feat
                            then ( (float countLookup.[feat]) / total)
                            else 0.)
...
let classifyLanguage text =
    ...
    languageFeatures
    |> Array.minBy (fun a -> distance featureVector (snd a))
    |> fst
...
//    language.
//languageFeatures
//|> Array.map (fun (lang, features) -> fun Array.sortBy (distance features) languageFeatures)
